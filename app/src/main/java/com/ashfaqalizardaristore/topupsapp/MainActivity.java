...
import com.ashfaqalizardaristore.topupsapp.databinding.ActivityMainBinding;
        ...
        billingClient.startConnection(new BillingClientStateListener() {
            ...
            public void onBillingSetupFinished(@NonNull BillingResult billingResult) {
                ...
                if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                    AtomicReference<List<Purchase>> p1 = null;
...
//
                    ...
                    if (p1.get() != null && p1.get().size() > 0) {
                        //check item in purchase list
                        for (Purchase p : p1.get()) {
                            int index = purchaseItemIDs.indexOf(p1);
                            //if purchase found
                            if (index > -1) {
        ...
        main.listview.setAdapter(arrayAdapter);
        notifyList();
                    ...
                    public void onSkuDetailsResponse(@NonNull BillingResult billingResult,
                        ...
                        if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                            if (skuDetailsList != null && skuDetailsList.size() > 0) {
                                ...
                                Toast.makeText(getApplicationContext(), billingResult.getResponseCode() + " Purchase Item " + PRODUCT_ID + " not Found", Toast.LENGTH_SHORT).show();
                            }
                        } else {
        ...
        //if item already purchased then check and reflect changes
            ...
            billingClient.queryPurchasesAsync(
                    QueryPurchasesParams.newBuilder()
                            .setProductType(BillingClient.ProductType.SUBS)
                            .build(),
                    (billingResult1, purchases1) -> {
                        // check billingResult
                        // process returned purchase list, e.g. display the plans user owns
                        if (purchases1 != null) {
                            handlePurchases(purchases1);
                        }
                    }
            );
...
//            }
    ...
    void handlePurchases(List<Purchase> purchases) {
        for (Purchase purchase : purchases) {
            ...
            final int index = purchaseItemIDs.indexOf(purchase);
            //purchase found
            if (index > -1) {
